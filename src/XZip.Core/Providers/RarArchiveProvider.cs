...
/// <summary>
/// Read-only RAR provider via SharpCompress.
/// RAR creation is not supported by SharpCompress, so <see cref="CreateAsync"/> throws.
/// </summary>
public sealed class RarArchiveProvider : IArchiveProvider
{
    ...
    public bool CanRead(string path) => Probe(path) == ArchiveFormat.Rar;

    public ArchiveFormat Probe(string path) =>
        FormatDetector.Detect(path) == ArchiveFormat.Rar ? ArchiveFormat.Rar : ArchiveFormat.Unknown;
    ...
    public Task<ArchiveHandle> OpenAsync(string path, string? password, CancellationToken cancellationToken)
    {
        ...
        var archive = RarArchive.Open(path, opts);
    ...
    {
        var archive = (RarArchive)handle.State;
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        var e = (RarArchiveEntry)entry.Token;
    ...
    public Task CreateAsync(
        ...
        IProgress<ArchiveProgress>? progress,
        CancellationToken cancellationToken)
        => throw new NotSupportedException("RAR creation is not supported by the bundled engine.");

    private static ArchiveEntry Map(RarArchiveEntry e) => new()
    {
